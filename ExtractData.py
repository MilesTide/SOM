    ...
    for k in range(len(object)):
        ...
        for j in range(len(data)):  #控制文件内部循环
            ...
            if(dateutil.parser.parse(fileDate)==dateutil.parser.parse(objectDate)):
                print(fileDate)
                if len(data) - j > 3:
                    if data[j + 3][4] - data[j][4] >= 0:#连续三天看涨
                        with open(csvFile, 'a', newline='') as f:  # 条件满足，记录数据
                            text = [objectName, data[j][0],1,data[j][2]-max(data[j][1],data[j][4]),
                                    min(data[j][1],data[j][4])-data[j][3],max(data[j][1],data[j][4])-min(data[j][1],
                                    data[j][4]),data[j][-5], data[j][-4], data[j][-3], data[j][-2], data[j][-1]]
                            csv.writer(f).writerow(text)
                        f.close()
                        break
                    if data[j + 3][4] - data[j][4] < 0:#连续三天看跌
                        with open(csvFile, 'a', newline='') as f:  # 条件满足，记录数据
                            text = [objectName, data[j][0], 0, data[j][2] - max(data[j][1], data[j][4]),
                                    min(data[j][1], data[j][4]) - data[j][3],
                                    max(data[j][1], data[j][4]) - min(data[j][1],
                                    data[j][4]), data[j][-5], data[j][-4],
                                    data[j][-3], data[j][-2], data[j][-1]]
                            csv.writer(f).writerow(text)
                        f.close()
                        break
